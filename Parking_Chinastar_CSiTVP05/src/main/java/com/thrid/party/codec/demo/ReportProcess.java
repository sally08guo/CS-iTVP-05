...
public class ReportProcess {
	...
	private void parkingStateInfo(byte[] binaryData) {
		...
		bMagneticField_X = Utilty.getInstance().byteArray2Int(binaryData[28],
				binaryData[29]);
		bMagneticField_Y = Utilty.getInstance().byteArray2Int(binaryData[30],
				binaryData[31]);
		bMagneticField_Z = Utilty.getInstance().byteArray2Int(binaryData[32],
				binaryData[33]);
		cMagneticField_X = Utilty.getInstance().byteArray2Int(binaryData[34],
				binaryData[35]);
		cMagneticField_Y = Utilty.getInstance().byteArray2Int(binaryData[36],
				binaryData[37]);
		cMagneticField_Z = Utilty.getInstance().byteArray2Int(binaryData[38],
				binaryData[39]);
	...
	private void deviceVersionInfo(byte[] binaryData) {
		...
		ResetStatusRegister = Utilty.getInstance().bytesToLong(binaryData, 18,
				1, true);
		ExceptionIdentification = Utilty.getInstance().bytesToLong(binaryData,
				19, 1, true);
		regularReporTime = Utilty.getInstance().bytesToLong(binaryData, 20, 2,
				true);

		ResetPosition = Utilty.getInstance().bytesToLong(binaryData, 22, 2,
				true);
		intervalTime = Utilty.getInstance().bytes2Int(binaryData, 24, 2);

		IMEI = new String(binaryData, 26, 16);
		IMSI = new String(binaryData, 42, 16);

		mxlevel= Utilty.getInstance().bytes2Int(binaryData, 58, 1);
		wcmx = Utilty.getInstance().bytesToLong(binaryData, 59, 1, true);
		ycmx = Utilty.getInstance().bytesToLong(binaryData, 60, 1, true);
	...
	public ObjectNode toJsonNode() {
		try {
			...
			// 根据msgType字段组装消息体
			if (this.msgType.equals(REQUEST)
					&& bFunctionCode == DEVICE_VERSION_INFO) {
				root.put("hasMore", this.hasMore);
				...
				deviceInfoData.put("TimelyReportInterval",
						this.regularReporTime);

				deviceInfoData.put("IntervalTime", this.intervalTime);
				deviceInfoData.put("ResetStatusRegister",
						numToHex16(this.ResetStatusRegister));
				deviceInfoData.put("ExceptionIdentificat",
						numToHex16(this.ExceptionIdentification));
				...
				deviceInfoData.put("DetectThresholdLevel",
						this.DetectThresholdLevel);
			...
			} else if (this.msgType.equals(REQUEST)
					&& bFunctionCode == TIMING_REPORT_PARKING) {
				root.put("hasMore", this.hasMore);
				ArrayNode arrynode = mapper.createArrayNode();
				...
				parkingTimingReportData.put("BatteryCapacity",
						this.batteryPercent);
				...
				parkingTimingReportData.put("BGMagneticField_X",
						this.bMagneticField_X);
				parkingTimingReportData.put("BGMagneticField_Y",
						this.bMagneticField_Y);
				parkingTimingReportData.put("BGMagneticField_Z",
						this.bMagneticField_Z);
				parkingTimingReportData.put("CurrentMagneticField_X",
						this.cMagneticField_X);
				parkingTimingReportData.put("CurrentMagneticField_Y",
						this.cMagneticField_Y);
				parkingTimingReportData.put("CurrentMagneticField_Z",
						this.cMagneticField_Z);

				parkingTimingReportNode.put("serviceData",
						parkingTimingReportData);
				...
				body.put("result",
						Utilty.getInstance().parseByte2HexStr(iResult));
				root.put("body", body);
			}
			return root;
		} catch (Exception e) {
